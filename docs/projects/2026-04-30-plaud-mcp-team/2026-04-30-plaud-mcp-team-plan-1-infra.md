...
6. **Plaud TOS — Bari engagement (Stefan + Bari):**
   - Capture full TOS HTML from `plaud.ai/terms` and adjacent pages (the research agent's WebFetch was blocked).
   - Hand to Bari Rascoe (Polsinelli) with the three flagged risk clauses from `spike/03-plaud-api-tos.md`: reverse-engineering, credential confidentiality, account-sharing. Strongest TOS-adjacent risk is the existence of Plaud's official OAuth API in private beta.
   - Bari verdict required before any non-Stefan user onboards. Stefan-only E2E testing (Phase 8 of design's rollout plan) can proceed in parallel with Bari review.
